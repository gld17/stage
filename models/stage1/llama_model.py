...
def group_query_attention(GQA_surrounding_path=None, GQA_kernel_path=None, include_backward=True):
    ...
    if GQA_kernel_path is None:
        GQA_kernel_path = "./sharding_spreadsheets/module/tpsp/group_query_attention_kernel_fused.csv"
...
def transformer_decoder_block(
    ffn_path=None, layernorm_path=None, residual_path=None, include_backward=True
):
    ...
    mha = ReplicateGraph.apply(
        group_query_attention(include_backward=include_backward), "mha.%s"
    )
...
def transformer_decoders(num_layers, decoder_template, include_backward=True):
    links = dict()
    decoders = list()
...
def llama(num_layers, embedding_path=None, regenerate=False, tpsp=False, include_backward=True):
    from . import CACHE_DIR
    import os

    cache_filename = os.path.join(
        CACHE_DIR, f"llama_{num_layers}_bwd{int(include_backward)}.csv"
    )
    ...
    decoders = transformer_decoders(num_layers, decoder_template, include_backward=include_backward)
