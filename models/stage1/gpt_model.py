...
def group_query_attention(
    GQA_surrounding_path=None, GQA_kernel_path=None, tpsp=True, include_backward=True
):
...
def transformer_decoder_block(
    ...
    if layernorm_path is None:
        ...
        if not tpsp:
            layernorm_path = (
                "./sharding_spreadsheets/module/tp_gpt/layer_norm.csv"
            )
    ...
    mha = ReplicateGraph.apply(
        group_query_attention(tpsp=tpsp, include_backward=include_backward), "mha.%s"
    )
...
def transformer_decoders(num_layers, decoder_template, tpsp=True, include_backward=True):
    links = dict()
    decoders = list()
...
def gpt(
    ...
    cache_filename = os.path.join(
        CACHE_DIR, f"gpt_{num_layers}_{tpsp}_bwd{int(include_backward)}.csv"
    )
    ...
    if embedding_path is None:
        ...
        if not tpsp:
            embedding_path = (
                "./sharding_spreadsheets/module/tp_gpt/embedding.csv"
            )
    ...
    decoder_template = transformer_decoder_block(
        tpsp=tpsp, include_backward=include_backward
    )
    decoders = transformer_decoders(
        num_layers, decoder_template, tpsp=tpsp, include_backward=include_backward
    )
        ...
        if tpsp:
            loss = ReplicateGraph.apply(
                TensorGraph.load_tensor_graph(
                    "./sharding_spreadsheets/module/tpsp_gpt/loss.csv"
                ),
                "loss.%s",
            )
        else:
            loss = ReplicateGraph.apply(
                TensorGraph.load_tensor_graph(
                    "./sharding_spreadsheets/module/tp_gpt/loss.csv"
                ),
                "loss.%s",
            )
        links = dict()
